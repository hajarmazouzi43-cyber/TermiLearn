      ...
      <nav style={{ backgroundColor: 'white', borderBottom: '1px solid #e2e8f0', padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 40 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => { setShowMissions(m => !m); setShowQuiz(false) }}
            style={{ padding: '8px 16px', borderRadius: 9, border: '1.5px solid #e2e8f0', backgroundColor: showMissions ? '#fefce8' : 'white', color: showMissions ? '#b45309' : '#64748b', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
          ...
          <button onClick={() => { setShowQuiz(q => !q); setShowMissions(false) }}
            style={{ padding: '8px 16px', borderRadius: 9, border: '1.5px solid #e2e8f0', backgroundColor: showQuiz ? '#f0fdf4' : 'white', color: showQuiz ? '#059669' : '#64748b', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>Quiz</button>
          ...
          <button onClick={() => setShowCopilot(c => !c)}
            style={{ padding: '8px 16px', borderRadius: 9, border: '1.5px solid #e2e8f0', backgroundColor: showCopilot ? '#eef2ff' : 'white', color: showCopilot ? '#6366f1' : '#64748b', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>🤖 AI Copilot</button>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {!showMissions && !showQuiz ? (
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              ...
              {/* Quick start commands en grille 2 colonnes */}
              <div style={{ marginTop: 40, textAlign: 'left' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 12 }}>
                  {[
                    ...
                    <div key={q.cmd}
                      style={{ backgroundColor: '#f8fafc', borderRadius: 10, padding: '10px 14px', border: '1.5px solid #f1f5f9', cursor: 'pointer', transition: 'all 0.15s' }}
                      onClick={() => setShowTerminalModal(true)}
                      onMouseEnter={e => { e.currentTarget.style.borderColor = '#10b981'; e.currentTarget.style.backgroundColor = '#f0fdf4' }}
                      onMouseLeave={e => { e.currentTarget.style.borderColor = '#f1f5f9'; e.currentTarget.style.backgroundColor = '#f8fafc' }}
                    >
            ...
            >
              <div style={{ backgroundColor: '#1e293b', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button onClick={() => setShowTerminalModal(false)} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: 18, lineHeight: 1, padding: '2px 6px', borderRadius: 4 }}
